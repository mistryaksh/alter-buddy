...
import { MentorSignInProps, MentorSignInValidationSchema } from "../../../../validation";
import { useAppDispatch } from "../../../../redux";
import { useMentorSignInMutation } from "../../../../redux/rtk-api";
import { handleError, handleMentorAuthentication, useLayoutSlice } from "../../../../redux/features";
...
export const MentorLoginPage = () => {
     ...
     useEffect(() => {
          ...
          if (isMentorSuccess) {
               ...
               if (localStore) {
                    dispatch(handleMentorAuthentication({ token: mentorData?.data.token as string }));
               }
               navigate("/mentor/dashboard");
          }
     }, [isMentorError, mentorError, dispatch, isMentorSuccess, mentorData, localStore, navigate]);
          ...
          <MainLayout hideNav>
               <div className="flex justify-center h-screen items-center bg-gradient-to-tr from-primary-100 to-white">
                    <div className="w-[50%] px-10 py-5 shadow-lg bg-white rounded-md flex flex-col">
                         <h6 className="text-2xl font-bold text-primary-500">Sign in to your account</h6>
                         <p className="text-gray-500 capitalize">
                              Problem while loging?{" "}
                              <span className="text-primary-500 underline">click here for how to?</span>
                         </p>
                         <Formik
                              ...
                              {({ handleBlur, handleChange, handleSubmit, touched, values, errors }) => (
                                   <form onSubmit={handleSubmit}>
                                        <div className="flex flex-col gap-5 mt-5">
                                             <p className="text-center text-red-500 uppercase text-sm">{error}</p>
                                             <TextField
                                             ...
                                             <TextField
                                                  ...
                                                  label="password"
                                             />
                                             <Link to="/mentor/reset-password" className="text-primary-500 underline">
                                                  Forgot password?
                                             </Link>
